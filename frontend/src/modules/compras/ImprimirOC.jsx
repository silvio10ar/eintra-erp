          ...
          <tbody>
            <tr>
              ...
              <td style={{width:'44%', verticalAlign:'top', paddingRight:8}}>
                <div style={{fontSize:'18pt', fontWeight:'900', color:'#1a3a5c', fontStyle:'italic', letterSpacing:'-1px', lineHeight:1}}>
                  E-INTRA
                </div>
                <div style={{fontSize:'8.5pt', fontWeight:'bold', marginTop:1}}>{EI.nombre}</div>
        ...
        <table className="items">
          <thead>
            <tr>
              ...
              <th style={{width:'5%'}}>UNID.<br/>MED.</th>
              <th style={{width: conTC ? '22%' : '28%'}}>DESCRIPCIÓN</th>
              <th style={{width:'8%'}}>PRECIO UNIT.<br/><span style={{color: colorMon === '#fff' ? '#adf' : '#adf'}}>{simb}</span></th>
              <th style={{width:'4%'}}>BONIF<br/>1</th>
              <th style={{width:'4%'}}>BONIF<br/>2</th>
              <th style={{width:'4%'}}>BONIF<br/>3</th>
              <th style={{width:'4%'}}>BONIF<br/>4</th>
              <th style={{width:'8%', background: colorMon, color:'#fff'}}>
                PRECIO UNIT.<br/>{simb}
              </th>
              {conTC && (
              ...
              <th style={{width:'8%'}}>PLAZO DE<br/>ENTREGA</th>
            </tr>
          </thead>
          <tbody>
            {filas.map(({ num, item }) =>
              item ? (
                <tr key={num}>
                  ...
                  <td style={{textAlign:'right', fontWeight:'bold', background:'#f0fff4', color: colorMon}}>
                    {fmtN(item.precio_final)}
                  </td>
                  {conTC && (
                    <td style={{textAlign:'right', color:'#7b4a00', background:'#fffde7'}}>
                      {fmtN(item.precio_final * oc.tasa_cambio)}
                    </td>
                  )}
                  <td style={{textAlign:'center', fontSize:'7pt'}}>{item.plazo}</td>
                </tr>
              ) : (
                <tr key={num} className="empty">
                  ...
                  <td/><td/><td/><td/><td/>
                  <td style={{textAlign:'right', color:'#ccc', background:'#fafffe'}}>—</td>
          ...
          <tfoot>
            <tr>
              <td colSpan={conTC ? 9 : 9} style={{textAlign:'right', background:'#e8eef5', color:'#1a3a5c', paddingRight:8}}>
                SUB-TOTAL SIN I.V.A.
              </td>
              <td style={{textAlign:'right', background: esUSD ? '#d4edda' : esEUR ? '#d4e5ed' : '#fce4e4', color: colorMon, fontSize:'8pt'}}>
                {simb} {fmtN(subtotal)}
              </td>
              {conTC && (
            ...
            <tr>
              <td style={{width:'32%', padding:'8px', verticalAlign:'bottom', borderRight:'1.5px solid #1a3a5c'}}>
                ...
                <div style={{borderTop:'1px solid #333', paddingTop:'3px', textAlign:'center', fontSize:'7.5pt'}}>
                  <div style={{fontWeight:'bold'}}>{oc.elaborado_por || ''}</div>
                  <div style={{fontSize:'6.5pt', color:'#555'}}>TÉCNICO E-INTRA SRL</div>
                </div>
              </td>
